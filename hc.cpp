...
bool hill(vector<vector<pair<int,int>>> &graph,vector<int> &hill_approx,int current_node, int goal,vector<int> &path) {
    ...
    for (auto &neighbor : neighbors) {
        ...
        if (hill(graph, hill_approx, next_node, goal, path)) return true;
    }
    path.pop_back();
    return false;
}
...
int main() {
    ...
    vector<int> path;
    if (hill(graph, hill_approx, 0, 6, path)) {
